...
cursor.execute("""
    CREATE OR REPLACE VIEW public.aparcamientos_distrito AS
    SELECT 
        distrito,
        count(distinct distrito) AS total_distritos,
        sum(habitantes) AS total_habitantes,
        sum(total) AS total_aparcamientos,
        sum(libres) AS total_aparcamientos_libres,
        COALESCE(sum(libres)::decimal / NULLIF(sum(habitantes), 0), 0) AS ratio_aparcamiento
    FROM public."aparcamiento"
    GROUP BY distrito;
